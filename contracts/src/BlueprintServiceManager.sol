...
contract CoSNARKsZkSaaSBlueprintServiceManager is BlueprintServiceManagerBase {
    ...
    // --- Events ---
    event CircuitRegistered(bytes32 indexed circuitId, address verifier, uint256 vkLength);
    event ProofVerified(bytes32 indexed circuitId, uint64 indexed jobCallId, bool success);
    ...
    function onJobResult(
        uint64 serviceId,
        uint8 job, // Corresponds to the job ID (casted)
        uint64 jobCallId,
        bytes calldata resultData,
        address operator // Address of the operator submitting the result
    )
        external
        override
        onlyFromMaster // Ensures only the Master MBSM can call this
    {
        if (job == REGISTER_CIRCUIT_JOB_ID) {
            _handleCircuitRegistrationResult(jobCallId, resultData);
        } else if (job == GENERATE_PROOF_JOB_ID) {
            _handleProofGenerationResult(serviceId, jobCallId, resultData);
        } else {
    ...
    function _handleCircuitRegistrationResult(uint64 jobCallId, bytes calldata resultData) internal {
    ...
    function _handleProofGenerationResult(uint64 serviceId, uint64 jobCallId, bytes calldata resultData) internal {
        ...
        (bytes memory circuitIdBytes, bytes memory proofBytes, bytes[] memory publicInputBytes) =
            Codec.decodeResult(resultData, (Field.Bytes, Field.Bytes, Field.BytesArray));
        ...
        bool success = verifier.verifyProof(vkInfo.verificationKey, proofBytes, publicInputBytes);
    ...
    function getVerificationKeyInfo(bytes32 circuitId) external view returns (VerificationKeyInfo memory) {
